...
// SaaS specific FAQs for SEO FAQPage schema
const saasFaqs = [
  {
    question: "How do you handle multi-tenant data security?",
    answer: "We employ strict Row-Level Security (RLS) in PostgreSQL via Supabase or Prisma, ensuring that tenant data is absolutely segregated. Each query is authenticated, guaranteeing users can only access data belonging to their specific organization."
  },
  {
    question: "Can you integrate Stripe for usage-based billing?",
    answer: "Yes, Stripe billing is a core competency. We integrate Stripe Checkout, Customer Portals, and Webhooks to manage metered usage, tiered subscriptions, prorations, and automated dunning (failed payment recovery)."
  },
  {
    question: "What tech stack do you use for SaaS products?",
    answer: "We rely on Next.js for server-side rendering, React for the frontend, Node.js for backend APIs, and PostgreSQL for robust relational data. This stack ensures high performance, excellent SEO for marketing pages, and rapid feature development."
  },
  {
    question: "Do you provide white-labeling capabilities?",
    answer: "Absolutely. We build SaaS architectures that support dynamic custom domain routing (using Vercel Edge functions) and theme customization, allowing your clients to brand the platform as their own."
  }
];
...
export default function SaasServiceDetail({ service }: { service: ServiceDetail }) {
  // Form State
  ...
  useEffect(() => {
    ...
    else if (projectType === "b2b") basePrice = 550000;
    else if (projectType === "b2c") basePrice = 450000;
    else if (projectType === "enterprise") basePrice = 900000;
    ...
    if (hasPayments) basePrice += 60000;
    if (hasDashboards) basePrice += 45000;
    if (hasWhiteLabel) basePrice += 80000;
    if (hasCustomDomain) basePrice += 30000;
  ...
  }, [projectType, pageCount, hasAuth, hasPayments, hasDashboards, hasWhiteLabel, hasCustomDomain, billingModel]);
...
- Dashboards & Analytics: ${hasDashboards ? "Yes" : "No"}
- White-labeling: ${hasWhiteLabel ? "Yes" : "No"}
- Custom Domains: ${hasCustomDomain ? "Yes" : "No"}
- Calculated Cost: ₹${estimatedCost.toLocaleString("en-IN")}`;
    ...
    if (hasDashboards) featuresList.push("Dashboards & Analytics");
    if (hasWhiteLabel) featuresList.push("White-labeling");
    if (hasCustomDomain) featuresList.push("Custom Domains");
  ...
  return (
    <div className="min-h-screen text-[var(--text-secondary)] bg-[var(--bg-primary)] relative overflow-hidden" style={customStyles}>
      ...
      <ServiceHero
        ...
        titleHighlight="Rapid Growth & Scalability"
        ...
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6">
          ...
          >
            <SectionHeader
              ...
              title="Why Single-Tenant Apps"
              titleHighlight="Fail Your Business"
              description="A side-by-side comparison of standard monolithic builds versus our highly scalable multi-tenant SaaS architectures."
              align="center"
              theme="dark"
            />
          </motion.div>
          ...
          >
            ...
            <div className="hidden md:flex items-start gap-8 relative z-10 w-full">
              ...
              <div className="w-[58.33%] pt-5">
                ...
                {/* Data Rows */}
                {[
                  { feature: "DATA SEGREGATION", value: "Prone to cross-tenant leaks" },
                  { feature: "INFRASTRUCTURE COSTS", value: "Expensive to host many users" },
                  { feature: "BILLING INTEGRATION", value: "Manual subscription updates" },
                  { feature: "CUSTOM DOMAINS", value: "Hardcoded & inflexible" },
                  { feature: "PERMISSIONS (RBAC)", value: "Basic admin/user roles" },
                  { feature: "ANALYTICS ENGINE", value: "Basic generic dashboards" },
                  { feature: "SCALING ABILITY", value: "Requires entire app rebuilds" }
                ].map((item, idx) => (
            ...
            <div className="block md:hidden space-y-4">
              {[
                { feature: "DATA SEGREGATION", traditional: "Prone to cross-tenant leaks", nextgen: "Strict Row-Level Security" },
                { feature: "INFRASTRUCTURE COSTS", traditional: "Expensive to host many users", nextgen: "Cost-effective shared infra" },
                { feature: "BILLING INTEGRATION", traditional: "Manual subscription updates", nextgen: "Automated Stripe gateways" },
                { feature: "CUSTOM DOMAINS", traditional: "Hardcoded & inflexible", nextgen: "Dynamic Edge routing" },
                { feature: "PERMISSIONS (RBAC)", traditional: "Basic admin/user roles", nextgen: "Granular team permissions" },
                { feature: "ANALYTICS ENGINE", traditional: "Basic generic dashboards", nextgen: "Embedded custom charts" },
                { feature: "SCALING ABILITY", traditional: "Requires entire app rebuilds", nextgen: "Infinite horizontal scale" }
              ].map((item, idx) => (
                <motion.div
                  key={idx}
              ...
              {calcStep === 1 && (
                <div className="space-y-5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {([
                      { id:"mvp",       label:"SaaS MVP",         desc:"Launch your idea to first customers",    base:250000, icon:"🚀" },
                      { id:"b2b",       label:"B2B Platform",     desc:"Team workspaces, RBAC, high security",   base:550000, icon:"🏢" },
                      { id:"b2c",       label:"B2C Portal",       desc:"High scale, social auth, fast DBs",      base:450000, icon:"👥" },
                      { id:"enterprise",label:"Enterprise SaaS",  desc:"Custom SLA, on-premise deployments",     base:900000, icon:"🌐" },
              ...
              {calcStep === 4 && (
                <div className="space-y-5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                    {([
                      { stateKey:"auth",     label:"Advanced RBAC & Auth",    sub:"SSO, Teams, Granular permissions",   price:40000, badge:"Security",   bc:"text-blue-600 bg-blue-50 border-blue-200" },
                      { stateKey:"payments", label:"Stripe Billing Hub",       sub:"Metered usage, proration, invoices", price:60000, badge:"Popular",    bc:"text-emerald-600 bg-emerald-50 border-emerald-200" },
                      { stateKey:"dashboards",label:"Custom Dashboards",       sub:"Recharts, admin views, data export", price:45000, badge:"Analytics",  bc:"text-amber-600 bg-amber-50 border-amber-200" },
                      { stateKey:"whitelabel",label:"White-label Features",    sub:"Brand colors, dynamic themes",       price:80000, badge:"Enterprise", bc:"text-purple-600 bg-purple-50 border-purple-200" },
                      { stateKey:"customdomain",label:"Custom Domains",        sub:"Vercel Edge CNAME mapping",          price:30000, badge:"Growth",     bc:"text-cyan-600 bg-cyan-50 border-cyan-200" },
                      ...
                      const map: Record<string, [boolean, (v:boolean)=>void]> = {
                        ...
                        dashboards:   [hasDashboards,   setHasDashboards],
                        whitelabel:   [hasWhiteLabel,   setHasWhiteLabel],
                        customdomain: [hasCustomDomain, setHasCustomDomain],
              ...
              {calcStep > 1 && (
                <div className="flex flex-wrap gap-2 mt-6 pt-5 border-t border-slate-100">
                  {projectType && (
                    ...
                    >
                      {projectType === "mvp" ? "SaaS MVP" : projectType === "b2b" ? "B2B Platform" : projectType === "b2c" ? "B2C Portal" : "Enterprise SaaS"}
                ...
                <div className="space-y-2 text-[12px] font-semibold text-white/90">
                  {projectType ? (
                    <div className="flex justify-between">
                      <span>{projectType === "mvp" ? "SaaS MVP" : projectType === "b2b" ? "B2B Platform" : projectType === "b2c" ? "B2C Portal" : "Enterprise SaaS"}</span>
                  ...
                  {hasAuth     && <div className="flex justify-between"><span>+ Advanced RBAC</span><span className="font-black font-mono">₹40,000</span></div>}
                  {hasPayments && <div className="flex justify-between"><span>+ Stripe Billing</span><span className="font-black font-mono">₹60,000</span></div>}
                  {hasDashboards && <div className="flex justify-between"><span>+ Dashboards</span><span className="font-black font-mono">₹45,000</span></div>}
                  {hasWhiteLabel && <div className="flex justify-between"><span>+ White-labeling</span><span className="font-black font-mono">₹80,000</span></div>}
                  {hasCustomDomain && <div className="flex justify-between"><span>+ Custom Domains</span><span className="font-black font-mono">₹30,000</span></div>}
