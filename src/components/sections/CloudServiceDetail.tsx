...
// Cloud specific FAQs for SEO FAQPage schema
const cloudFaqs = [
  {
    question: "How do you ensure zero-downtime migrations?",
    answer: "We use Infrastructure as Code (Terraform) to spin up identical cloud environments alongside your current setup. We sync the database in real-time and switch DNS records seamlessly, ensuring no traffic drops."
  },
  {
    question: "Can you optimize our existing AWS/GCP bill?",
    answer: "Yes, we run comprehensive audits to identify underutilized resources, implement auto-scaling rules, transition to serverless where applicable, and leverage spot instances to significantly reduce monthly cloud spend."
  },
  {
    question: "What compliance standards do your cloud setups meet?",
    answer: "We design architectures that are SOC2, HIPAA, and GDPR compliant out of the box, utilizing private VPCs, encrypted storage buckets, strict audit logs, and granular IAM roles."
  },
  {
    question: "Do you provide 24/7 infrastructure monitoring?",
    answer: "Absolutely. We set up automated anomaly alerts using Datadog, CloudWatch, or Grafana, allowing our on-call engineers to react instantly to traffic spikes, database loads, or security threats."
  }
];
...
export default function CloudServiceDetail({ service }: { service: ServiceDetail }) {
  // Form State
  ...
  const [hasTerraform, setHasTerraform] = useState(true);
  ...
  const [hasCdn, setHasCdn] = useState(false);
  ...
  useEffect(() => {
    ...
    if (projectType === "mvp") basePrice = 200000;
    else if (projectType === "b2b") basePrice = 500000;
    else if (projectType === "b2c") basePrice = 650000;
    else if (projectType === "enterprise") basePrice = 950000;

    // Pages cost
    basePrice += pageCount * 12000;

    // Feature toggles
    if (hasTerraform) basePrice += 60000;
    if (hasDocker) basePrice += 80000;
    if (hasMonitoring) basePrice += 35000;
    if (hasSoc2) basePrice += 90000;
    if (hasCdn) basePrice += 40000;
  ...
  }, [projectType, pageCount, hasTerraform, hasDocker, hasMonitoring, hasSoc2, hasCdn, billingModel]);
...
- Pages: ${pageCount}
- Terraform IaC: ${hasTerraform ? "Yes" : "No"}
- Docker Containerization: ${hasDocker ? "Yes" : "No"}
- 24/7 Monitoring: ${hasMonitoring ? "Yes" : "No"}
- SOC2 Compliance: ${hasSoc2 ? "Yes" : "No"}
- Global Edge CDN: ${hasCdn ? "Yes" : "No"}
- Calculated Cost: ₹${estimatedCost.toLocaleString("en-IN")}`;
    
    const featuresList: string[] = [];
    if (hasTerraform) featuresList.push("Terraform IaC");
    if (hasDocker) featuresList.push("Docker Containerization");
    if (hasMonitoring) featuresList.push("24/7 Monitoring");
    if (hasSoc2) featuresList.push("SOC2 Compliance Setup");
    if (hasCdn) featuresList.push("Global Edge CDN");
  ...
  return (
    <div className="min-h-screen text-[var(--text-secondary)] bg-[var(--bg-primary)] relative overflow-hidden" style={customStyles}>
      ...
      <ServiceHero
        title="Enterprise Cloud Architecture Built For"
        titleHighlight="Zero Downtime & Global Scale"
        ...
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6">
          ...
          >
            <SectionHeader
              ...
              title="Why Traditional Hosting"
              titleHighlight="Costs You Money"
              description="A side-by-side comparison of standard bare-metal servers versus our elastic, auto-scaling cloud architectures."
              align="center"
              theme="dark"
            />
          </motion.div>
          ...
          >
            ...
            <div className="hidden md:flex items-start gap-8 relative z-10 w-full">
              ...
              <div className="w-[58.33%] pt-5">
                ...
                {/* Data Rows */}
                {[
                  { feature: "SERVER DEPLOYMENT", value: "Manual provisioning (Days)" },
                  { feature: "TRAFFIC SCALING", value: "Hard limits & crashes" },
                  { feature: "DATA BACKUPS", value: "Manual cron jobs" },
                  { feature: "SECURITY POSTURE", value: "Perimeter firewalls only" },
                  { feature: "COST STRUCTURE", value: "Fixed high monthly fees" },
                  { feature: "DISASTER RECOVERY", value: "Hours or days to restore" },
                  { feature: "GLOBAL PERFORMANCE", value: "Single datacenter lag" }
                ].map((item, idx) => (
              ...
              >
                ...
                >
                  ...
                  {[
                    ...
                    "Automated multi-region snapshots",
                    "Zero-trust & VPC isolation",
                    "Pay-only-for-what-you-use",
                    "Instant automated failovers",
            ...
            <div className="block md:hidden space-y-4">
              {[
                { feature: "SERVER DEPLOYMENT", traditional: "Manual provisioning (Days)", nextgen: "Infrastructure as Code (Minutes)" },
                { feature: "TRAFFIC SCALING", traditional: "Hard limits & crashes", nextgen: "Elastic Auto-Scaling" },
                { feature: "DATA BACKUPS", traditional: "Manual cron jobs", nextgen: "Automated multi-region snapshots" },
                { feature: "SECURITY POSTURE", traditional: "Perimeter firewalls only", nextgen: "Zero-trust & VPC isolation" },
                { feature: "COST STRUCTURE", traditional: "Fixed high monthly fees", nextgen: "Pay-only-for-what-you-use" },
                { feature: "DISASTER RECOVERY", traditional: "Hours or days to restore", nextgen: "Instant automated failovers" },
                { feature: "GLOBAL PERFORMANCE", traditional: "Single datacenter lag", nextgen: "Global Edge CDN routing" }
              ].map((item, idx) => (
                <motion.div
                  key={idx}
              ...
              {calcStep === 1 && (
                <div className="space-y-5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {([
                      { id:"mvp",       label:"Cloud Migration",  desc:"Lift and shift to secure cloud",        base:200000, icon:"☁️" },
                      { id:"b2b",       label:"Auto-Scaling",     desc:"Load balancers & elastic clusters",     base:500000, icon:"📈" },
                      { id:"b2c",       label:"Serverless",       desc:"Microservices & Lambda functions",      base:650000, icon:"⚡" },
                      { id:"enterprise",label:"Hybrid Cloud",     desc:"Multi-region SOC2 compliance",          base:950000, icon:"🏢" },
              ...
              {calcStep === 4 && (
                <div className="space-y-5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                    {([
                      { stateKey:"auth",     label:"Terraform IaC",           sub:"Infrastructure as Code automation",  price:60000, badge:"Security",   bc:"text-blue-600 bg-blue-50 border-blue-200" },
                      { stateKey:"payments", label:"Docker & Kubernetes",     sub:"Containerized deployments",          price:80000, badge:"Popular",    bc:"text-emerald-600 bg-emerald-50 border-emerald-200" },
                      { stateKey:"dashboards",label:"24/7 Monitoring",        sub:"Datadog / CloudWatch alerts",        price:35000, badge:"Analytics",  bc:"text-amber-600 bg-amber-50 border-amber-200" },
                      { stateKey:"whitelabel",label:"SOC2 Compliance",        sub:"Audit logs, VPC isolation, IAM",     price:90000, badge:"Enterprise", bc:"text-purple-600 bg-purple-50 border-purple-200" },
                      { stateKey:"customdomain",label:"Global Edge CDN",      sub:"Cloudflare edge routing, WAF",       price:40000, badge:"Growth",     bc:"text-cyan-600 bg-cyan-50 border-cyan-200" },
                      ...
                      const map: Record<string, [boolean, (v:boolean)=>void]> = {
                        auth:         [hasTerraform,         setHasTerraform],
                        payments:     [hasDocker,     setHasDocker],
                        dashboards:   [hasMonitoring,   setHasMonitoring],
                        whitelabel:   [hasSoc2,   setHasSoc2],
                        customdomain: [hasCdn, setHasCdn],
                        seo:          [hasTerraform,         setHasTerraform],
                      };
              ...
              {calcStep > 1 && (
                <div className="flex flex-wrap gap-2 mt-6 pt-5 border-t border-slate-100">
                  {projectType && (
                    ...
                    >
                      {projectType === "mvp" ? "Cloud Migration" : projectType === "b2b" ? "Auto-Scaling Setup" : projectType === "b2c" ? "Serverless Architect" : "Enterprise Hybrid"}
                ...
                <div className="space-y-2 text-[12px] font-semibold text-white/90">
                  {projectType ? (
                    <div className="flex justify-between">
                      <span>{projectType === "mvp" ? "Cloud Migration" : projectType === "b2b" ? "Auto-Scaling Setup" : projectType === "b2c" ? "Serverless Architect" : "Enterprise Hybrid"}</span>
                  ...
                  {hasTerraform && <div className="flex justify-between"><span>+ Terraform IaC</span><span className="font-black font-mono">₹60,000</span></div>}
                  {hasDocker && <div className="flex justify-between"><span>+ Docker & K8s</span><span className="font-black font-mono">₹80,000</span></div>}
                  {hasMonitoring && <div className="flex justify-between"><span>+ 24/7 Monitoring</span><span className="font-black font-mono">₹35,000</span></div>}
                  {hasSoc2 && <div className="flex justify-between"><span>+ SOC2 Compliance</span><span className="font-black font-mono">₹90,000</span></div>}
                  {hasCdn && <div className="flex justify-between"><span>+ Global Edge CDN</span><span className="font-black font-mono">₹40,000</span></div>}
