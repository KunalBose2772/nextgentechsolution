...
// Social Media Marketing specific FAQs for SEO FAQPage schema
const smmFaqs = [
  {
    question: "How do you decide what to post?",
    answer: "We define strict brand guidelines and content pillars upfront. Every post, video, and graphic is designed to hit specific metrics—whether it’s awareness, engagement, or lead generation."
  },
  {
    question: "Do you handle video editing for Reels/Shorts?",
    answer: "Yes! Our in-house creative team edits raw footage into high-retention, fast-paced vertical videos optimized for the algorithms of Instagram, TikTok, and YouTube Shorts."
  },
  {
    question: "How do we measure success?",
    answer: "We track more than just vanity metrics. Our monthly reports dive deep into reach, engagement rates, profile clicks, and actual conversions derived from our organic campaigns."
  },
  {
    question: "Can you manage our LinkedIn executive profiles?",
    answer: "Absolutely. We specialize in B2B ghostwriting and personal branding, helping executives build thought leadership and drive high-value networking on LinkedIn."
  }
];
...
export default function SmmServiceDetail({ service }: { service: ServiceDetail }) {
  // Form State
  ...
  const [hasInfluencer, setHasInfluencer] = useState(false);
  ...
  useEffect(() => {
    ...
    if (projectType === "mvp") basePrice = 65000;
    else if (projectType === "b2b") basePrice = 130000;
    ...
    // Feature toggles
    if (hasVideo) basePrice += 45000;
    if (hasCommunity) basePrice += 30000;
    if (hasLinkedin) basePrice += 50000;
    if (hasDesign) basePrice += 25000;
    if (hasInfluencer) basePrice += 60000;
  ...
  }, [projectType, pageCount, hasVideo, hasCommunity, hasLinkedin, hasDesign, hasInfluencer, billingModel]);
...
- Calculated Cost: ₹${estimatedCost.toLocaleString("en-IN")}`;
  ...
  return (
    <div className="min-h-screen text-[var(--text-secondary)] bg-[var(--bg-primary)] relative overflow-hidden" style={customStyles}>
      ...
      <ServiceHero
        title="Organic Social Media Growth Built For"
        titleHighlight="Viral Reach & Brand Loyalty"
        ...
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6">
          ...
          >
            <SectionHeader
              ...
              title="Why Traditional Social Media"
              titleHighlight="Doesn't Convert"
              description="A side-by-side comparison of standard social media posting versus our highly engaging, conversion-focused content strategy."
              align="center"
              theme="dark"
            />
          </motion.div>
          ...
          >
            ...
            <div className="hidden md:flex items-start gap-8 relative z-10 w-full">
              ...
              <div className="w-[58.33%] pt-5">
                ...
                {/* Data Rows */}
                {[
                  { feature: "CONTENT STRATEGY", value: "Random generic quotes" },
                  { feature: "VIDEO PRODUCTION", value: "Boring corporate slides" },
                  { feature: "COMMUNITY", value: "Ignoring comments & DMs" },
                  { feature: "PLATFORM FOCUS", value: "Cross-posting same formats" },
                  { feature: "COPYWRITING", value: "Bland AI-generated captions" },
                  { feature: "REPORTING", value: "Vanity follower metrics" },
                  { feature: "BRAND VOICE", value: "Soulless and corporate" }
                ].map((item, idx) => (
              ...
              >
                ...
                >
                  ...
                  {[
                    ...
                    "Native algorithm optimization",
                    "Hook-driven viral hooks",
                    "Revenue & click tracking",
                    "Authentic & personality-led"
                  ].map((value, idx) => (
            ...
            <div className="block md:hidden space-y-4">
              {[
                { feature: "CONTENT STRATEGY", traditional: "Random generic quotes", nextgen: "Data-driven content pillars" },
                { feature: "VIDEO PRODUCTION", traditional: "Boring corporate slides", nextgen: "High-retention vertical reels" },
                { feature: "COMMUNITY", traditional: "Ignoring comments & DMs", nextgen: "Active daily engagement" },
                { feature: "PLATFORM FOCUS", traditional: "Cross-posting same formats", nextgen: "Native algorithm optimization" },
                { feature: "COPYWRITING", traditional: "Bland AI-generated captions", nextgen: "Hook-driven viral hooks" },
                { feature: "REPORTING", traditional: "Vanity follower metrics", nextgen: "Revenue & click tracking" },
                { feature: "BRAND VOICE", traditional: "Soulless and corporate", nextgen: "Authentic & personality-led" }
              ].map((item, idx) => (
                <motion.div
                  key={idx}
              ...
              {calcStep === 1 && (
                <div className="space-y-5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {([
                      { id:"mvp",       label:"Starter Presence", desc:"Basic scheduling & posts",              base:65000,  icon:"📅" },
                      { id:"b2b",       label:"B2B Authority",    desc:"LinkedIn & X ghostwriting",             base:130000, icon:"✍️" },
                      { id:"b2c",       label:"Viral Video",      desc:"Reels, TikToks, Shorts",                base:220000, icon:"📱" },
                      { id:"enterprise",label:"Omnichannel",      desc:"Full-scale brand management",           base:450000, icon:"🌐" },
              ...
              {calcStep === 4 && (
                <div className="space-y-5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                    {([
                      { stateKey:"auth",     label:"Vertical Video Editing",  sub:"4 custom reels/shorts per month",    price:45000, badge:"Standard",   bc:"text-blue-600 bg-blue-50 border-blue-200" },
                      { stateKey:"payments", label:"Daily Community Mgmt",    sub:"Active comments & DM replies",       price:30000, badge:"Popular",    bc:"text-emerald-600 bg-emerald-50 border-emerald-200" },
                      { stateKey:"dashboards",label:"Exec Ghostwriting",      sub:"LinkedIn thought leadership posts",  price:50000, badge:"Analytics",  bc:"text-amber-600 bg-amber-50 border-amber-200" },
                      { stateKey:"whitelabel",label:"Custom Graphic Design",  sub:"Infographics & premium carousels",   price:25000, badge:"Enterprise", bc:"text-purple-600 bg-purple-50 border-purple-200" },
                      { stateKey:"customdomain",label:"Influencer Outreach",  sub:"Nano/micro influencer partnerships", price:60000, badge:"Growth",     bc:"text-cyan-600 bg-cyan-50 border-cyan-200" },
                      ...
                      const map: Record<string, [boolean, (v:boolean)=>void]> = {
                        auth:         [hasVideo,      setHasVideo],
                        payments:     [hasCommunity,  setHasCommunity],
                        dashboards:   [hasLinkedin,   setHasLinkedin],
                        whitelabel:   [hasDesign,     setHasDesign],
                        customdomain: [hasInfluencer, setHasInfluencer],
                        seo:          [hasVideo,      setHasVideo],
                      };
              ...
              {calcStep > 1 && (
                <div className="flex flex-wrap gap-2 mt-6 pt-5 border-t border-slate-100">
                  {projectType && (
                    ...
                    >
                      {projectType === "mvp" ? "Starter Presence" : projectType === "b2b" ? "B2B Authority" : projectType === "b2c" ? "Viral Video" : "Omnichannel"}
                ...
                <div className="space-y-2 text-[12px] font-semibold text-white/90">
                  {projectType ? (
                    <div className="flex justify-between">
                      <span>{projectType === "mvp" ? "Starter Presence" : projectType === "b2b" ? "B2B Authority" : projectType === "b2c" ? "Viral Video" : "Omnichannel"}</span>
                  ...
                  {hasVideo && <div className="flex justify-between"><span>+ Vertical Video Editing</span><span className="font-black font-mono">₹45,000</span></div>}
                  {hasCommunity && <div className="flex justify-between"><span>+ Community Mgmt</span><span className="font-black font-mono">₹30,000</span></div>}
                  {hasLinkedin && <div className="flex justify-between"><span>+ Exec Ghostwriting</span><span className="font-black font-mono">₹50,000</span></div>}
                  {hasDesign && <div className="flex justify-between"><span>+ Custom Graphics</span><span className="font-black font-mono">₹25,000</span></div>}
                  {hasInfluencer && <div className="flex justify-between"><span>+ Influencer Outreach</span><span className="font-black font-mono">₹60,000</span></div>}
