...
// PPC specific FAQs for SEO FAQPage schema
const ppcFaqs = [
  {
    question: "What is ROAS and why does it matter?",
    answer: "Return on Ad Spend measures the exact revenue generated for every dollar spent. We optimize your campaigns to ensure positive ROAS, meaning your ads are profitable, not just burning cash."
  },
  {
    question: "Which platforms do you run ads on?",
    answer: "We handle Google Search for high-intent keywords, Meta (Facebook/Instagram) for visual disruption and retargeting, and LinkedIn for high-ticket B2B lead generation."
  },
  {
    question: "Do you design the ad creatives?",
    answer: "Yes, our team handles the entire creative pipeline—from writing high-converting ad copy to designing the image and video assets needed for A/B testing."
  },
  {
    question: "How do you track conversions?",
    answer: "We implement advanced server-side tracking, Facebook Conversions API, and Google Tag Manager so every form fill, purchase, and phone call is accurately attributed to the correct ad."
  }
];
...
export default function PpcServiceDetail({ service }: { service: ServiceDetail }) {
  // Form State
  ...
  const [hasLinkedInAds, setHasLinkedInAds] = useState(false);
  ...
  useEffect(() => {
    ...
    if (projectType === "mvp") basePrice = 80000;
    else if (projectType === "b2b") basePrice = 150000;
    else if (projectType === "b2c") basePrice = 250000;
    else if (projectType === "enterprise") basePrice = 500000;

    // Pages cost
    basePrice += pageCount * 12000;

    // Feature toggles
    if (hasGoogleAds) basePrice += 40000;
    if (hasMetaAds) basePrice += 40000;
    if (hasLinkedInAds) basePrice += 50000;
    if (hasTracking) basePrice += 35000;
    if (hasRetargeting) basePrice += 30000;
  ...
  }, [projectType, pageCount, hasGoogleAds, hasMetaAds, hasLinkedInAds, hasTracking, hasRetargeting, billingModel]);
...
- Pages: ${pageCount}
- Google Search Ads: ${hasGoogleAds ? "Yes" : "No"}
- Meta Visual Ads: ${hasMetaAds ? "Yes" : "No"}
- LinkedIn B2B Ads: ${hasLinkedInAds ? "Yes" : "No"}
- Advanced Pixel Tracking: ${hasTracking ? "Yes" : "No"}
- Dynamic Retargeting: ${hasRetargeting ? "Yes" : "No"}
- Calculated Cost: ₹${estimatedCost.toLocaleString("en-IN")}`;
    
    const featuresList: string[] = [];
    if (hasGoogleAds) featuresList.push("Google Search Ads");
    if (hasMetaAds) featuresList.push("Meta Visual Ads");
    if (hasLinkedInAds) featuresList.push("LinkedIn B2B Ads");
    if (hasTracking) featuresList.push("Advanced Pixel Tracking");
    if (hasRetargeting) featuresList.push("Dynamic Retargeting");
  ...
  return (
    <div className="min-h-screen text-[var(--text-secondary)] bg-[var(--bg-primary)] relative overflow-hidden" style={customStyles}>
      ...
      <ServiceHero
        title="Paid Advertising Campaigns Built For"
        titleHighlight="Direct Conversions & High ROAS"
        ...
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6">
          ...
          >
            <SectionHeader
              ...
              title="Why Simply Boosting Posts"
              titleHighlight="Burns Your Budget"
              description="A side-by-side comparison of amateur ad boosting versus our highly targeted, conversion-focused paid media campaigns."
              align="center"
              theme="dark"
            />
          </motion.div>
          ...
          >
            ...
            <div className="hidden md:flex items-start gap-8 relative z-10 w-full">
              ...
              <div className="w-[58.33%] pt-5">
                ...
                {/* Data Rows */}
                {[
                  { feature: "AUDIENCE TARGETING", value: "Broad age/location limits" },
                  { feature: "CONVERSION TRACKING", value: "Likes and page views" },
                  { feature: "AD CREATIVES", value: "Single static image" },
                  { feature: "BUDGET SCALING", value: "Guessing and burning cash" },
                  { feature: "RETARGETING", value: "Non-existent or broken" },
                  { feature: "PLATFORM ALGORITHM", value: "Fighting the algorithm" },
                  { feature: "REPORTING", value: "Confusing dashboard metrics" }
                ].map((item, idx) => (
              ...
              >
                ...
                >
                  ...
                  {/* Card Rows */}
                  {[
                    "Custom lookalike & pixel audiences",
                    "Server-side purchase tracking",
                    "Dynamic A/B multivariate testing",
                    "Scaling only profitable ad sets",
                    "Multi-channel funnel loops",
                    "Feeding AI with conversion data",
                    "Clear Cost-Per-Acquisition (CPA)"
                  ].map((value, idx) => (
            ...
            <div className="block md:hidden space-y-4">
              {[
                { feature: "AUDIENCE TARGETING", traditional: "Broad age/location limits", nextgen: "Custom lookalike & pixel audiences" },
                { feature: "CONVERSION TRACKING", traditional: "Likes and page views", nextgen: "Server-side purchase tracking" },
                { feature: "AD CREATIVES", traditional: "Single static image", nextgen: "Dynamic A/B multivariate testing" },
                { feature: "BUDGET SCALING", traditional: "Guessing and burning cash", nextgen: "Scaling only profitable ad sets" },
                { feature: "RETARGETING", traditional: "Non-existent or broken", nextgen: "Multi-channel funnel loops" },
                { feature: "PLATFORM ALGORITHM", traditional: "Fighting the algorithm", nextgen: "Feeding AI with conversion data" },
                { feature: "REPORTING", traditional: "Confusing dashboard metrics", nextgen: "Clear Cost-Per-Acquisition (CPA)" }
              ].map((item, idx) => (
                <motion.div
                  key={idx}
              ...
              {calcStep === 1 && (
                <div className="space-y-5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {([
                      { id:"mvp",       label:"Validation Test",  desc:"Testing initial ad sets",               base:80000,  icon:"🧪" },
                      { id:"b2b",       label:"B2B Lead Gen",     desc:"LinkedIn & Google Search",              base:150000, icon:"🏢" },
                      { id:"b2c",       label:"E-Commerce Scale", desc:"ROAS optimized catalog ads",            base:250000, icon:"🛒" },
                      { id:"enterprise",label:"Omnichannel",      desc:"Multi-platform dominance",              base:500000, icon:"📈" },
              ...
              {calcStep === 4 && (
                <div className="space-y-5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                    {([
                      { stateKey:"auth",     label:"Google Search Ads",       sub:"High-intent keyword targeting",      price:40000, badge:"Standard",   bc:"text-blue-600 bg-blue-50 border-blue-200" },
                      { stateKey:"payments", label:"Meta Visual Ads",         sub:"Facebook & Instagram placements",    price:40000, badge:"Popular",    bc:"text-emerald-600 bg-emerald-50 border-emerald-200" },
                      { stateKey:"dashboards",label:"LinkedIn B2B Ads",       sub:"Account-based marketing (ABM)",      price:50000, badge:"Analytics",  bc:"text-amber-600 bg-amber-50 border-amber-200" },
                      { stateKey:"whitelabel",label:"Advanced Pixel Tracking",sub:"Server-side conversion API",         price:35000, badge:"Enterprise", bc:"text-purple-600 bg-purple-50 border-purple-200" },
                      { stateKey:"customdomain",label:"Dynamic Retargeting",  sub:"Multi-channel follow-up loops",      price:30000, badge:"Growth",     bc:"text-cyan-600 bg-cyan-50 border-cyan-200" },
                      ...
                      const map: Record<string, [boolean, (v:boolean)=>void]> = {
                        auth:         [hasGoogleAds,   setHasGoogleAds],
                        payments:     [hasMetaAds,     setHasMetaAds],
                        dashboards:   [hasLinkedInAds, setHasLinkedInAds],
                        whitelabel:   [hasTracking,    setHasTracking],
                        customdomain: [hasRetargeting, setHasRetargeting],
                        seo:          [hasGoogleAds,   setHasGoogleAds],
                      };
              ...
              {calcStep > 1 && (
                <div className="flex flex-wrap gap-2 mt-6 pt-5 border-t border-slate-100">
                  {projectType && (
                    ...
                    >
                      {projectType === "mvp" ? "Validation Test" : projectType === "b2b" ? "B2B Lead Gen" : projectType === "b2c" ? "E-Commerce Scale" : "Omnichannel Growth"}
                ...
                <div className="space-y-2 text-[12px] font-semibold text-white/90">
                  {projectType ? (
                    <div className="flex justify-between">
                      <span>{projectType === "mvp" ? "Validation Test" : projectType === "b2b" ? "B2B Lead Gen" : projectType === "b2c" ? "E-Commerce Scale" : "Omnichannel Growth"}</span>
                  ...
                  {hasGoogleAds && <div className="flex justify-between"><span>+ Google Search Ads</span><span className="font-black font-mono">₹40,000</span></div>}
                  {hasMetaAds && <div className="flex justify-between"><span>+ Meta Visual Ads</span><span className="font-black font-mono">₹40,000</span></div>}
                  {hasLinkedInAds && <div className="flex justify-between"><span>+ LinkedIn B2B Ads</span><span className="font-black font-mono">₹50,000</span></div>}
                  {hasTracking && <div className="flex justify-between"><span>+ Advanced Pixel Tracking</span><span className="font-black font-mono">₹35,000</span></div>}
                  {hasRetargeting && <div className="flex justify-between"><span>+ Dynamic Retargeting</span><span className="font-black font-mono">₹30,000</span></div>}
